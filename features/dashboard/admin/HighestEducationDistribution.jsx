...
function EducationBarChart({ data }) {
    const chartData = useMemo(() => {
        return data.map((d) => ({
            name: d?.highest_education ?? "N/A",
...
export function HighestEducationDistribution() {
    ...
    useEffect(() => {
        ...
        async function run() {
            try {
                const res = await ReportService.getHighestEducationCounts();
                if (!mounted) return;
                setData(Array.isArray(res) ? res : []);
            } catch (e) {
                console.error("Education Chart Error:", e);
            } finally {
    ...
    const handleExportPdf = () => {
        ...
        doc.setFontSize(14);
        doc.text("Highest Educational Attainment Report", 14, 20);
        ...
        autoTable(doc, {
            startY: 30,
            head: [["Educational Level", "Total Count"]],
            body: tableBody,
            theme: "grid",
            styles: {
                font: "helvetica",
        ...
        doc.save("highest-educational-attainment-report.pdf");
    };
