...
function PlacementBarChart({ data }) {
    const chartData = useMemo(() => {
        return data.map((d) => ({
            name: d?.placement ?? "N/A",
...
export function PlacementDistribution() {
    ...
    useEffect(() => {
        ...
        async function run() {
            try {
                const res = await ReportService.getPlacementCounts();
                if (!mounted) return;
                setData(Array.isArray(res) ? res : []);
            } catch (e) {
                console.error("Placement Chart Error:", e);
            } finally {
    ...
    const handleExportPdf = () => {
        ...
        doc.setFontSize(14);
        doc.text("Placement Distribution Report", 14, 20);
        ...
        autoTable(doc, {
            startY: 30,
            head: [["Placement Type", "Total Count"]],
            body: tableBody,
            theme: "grid",
            styles: {
                font: "helvetica",
        ...
        doc.save("placement-distribution-report.pdf");
    };
    ...
    return (
        <section className="w-full">
            ...
            <div className="mt-4 px-2">
                {selectedTab === "table" && <PlacementTable data={data} />}
                {selectedTab === "graph" && <PlacementBarChart data={data} />}
            </div>
        </section>
    );
}
